...
export function getBoards(boardId: number, startRange?: number, endRange?: number, orderBy?: string, ascending?: boolean): Promise<ApiModels.Board[]> {
...
export function getBoard(boardId: number): Promise<ApiModels.Board[]> {
    return HttpClient.get(`${BASE_URL}boardId/${boardId}`)
        .then(response => response.json() as Promise<ApiModels.Board[]>);
};